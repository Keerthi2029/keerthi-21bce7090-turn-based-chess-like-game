...
function GameBoard() {
    ...
    useEffect(() => {
        const socket = new WebSocket('wss://keerthi-21bce7090-server-chess-board.onrender.com');
        setWs(socket);
    ...
    const resetGame = () => {
        if (ws && ws.readyState === WebSocket.OPEN) {
            ...
            // Request the latest game state after a short delay to ensure the server has processed the reset
            setTimeout(() => {
